...
module Key_value : sig
  ...
  val key_compare : ('a -> 'a -> Arith.int) -> 'a -> 'a -> compare_t
...
let rec key_compare
  ord k1 k2 =
    let n = ord k1 k2 in
    (if Arith.less_int n Arith.zero_int then LT
      else (if Arith.equal_int n Arith.zero_int then EQ else GT));;
  ...
  s1 s2 =
    Util.assert_true
      (let (_, fs1) = s1 in
       let (_, fs2) = s2 in
       (match (fs1, fs2)
         with (F_down (_, (_, (_, stk))), F_down (_, (_, (_, stka)))) ->
           Arith.equal_nata (List.size_list stka)
             (Arith.plus_nat Arith.one_nat (List.size_list stk))
         | (F_down (_, (_, (_, _))), F_finished _) -> true
         | (F_finished _, _) -> false));;
...
let rec wf_d
  k_ord r2t t0 s d =
    Util.assert_true
      (let (fs, _) = d in
       Find_state.wellformed_find_state k_ord r2t t0 s fs);;
...
let rec wf_u
  r2t k_ord t0 s k v u =
    Util.assert_true
      (let _ = Find_state.wf_store_tree r2t s in
       let check_stack =
         (fun rstk tstk ->
           Tree_stack.rstack_equal
             (Util.rev_apply
...
let rec wf_d
  k_ord r2f t0 s d =
    Util.assert_true
      (let (fs, _) = d in
       Util.assert_true (Find_state.wellformed_find_state k_ord r2f t0 s fs));;
              ...
              | false ->
                (match existinga
                  with [] ->
                    let _ =
                      Util.assert_true
                        (Arith.equal_nata len_existing Arith.zero_nat)
                      in
                    Some ((k, v) :: result,
                           (Arith.plus_nat len_result Arith.one_nat,
                             (newaa, (existinga, len_existing))))
                  | (ka, va) :: existingaa ->
                    (match Key_value.key_compare k_ord k ka
                      with Key_value.LT ->
                        Some ((k, v) :: result,
                               (Arith.plus_nat len_result Arith.one_nat,
                                 (newaa, (existinga, len_existing))))
