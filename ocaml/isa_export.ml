...
module Leaf_stream_state : sig
  type ('a, 'b, 'c) ls_state =
    LS_down of
      ('c * ('a, 'c, unit) Searching_and_splitting.rsplit_node_ext list)
    | LS_leaf of
        (('a * 'b) list *
...
module Insert_many_state : sig
  type ('a, 'b, 'c) im_fo = IM1 of ('c * ('a * 'b) list) |
    IM2 of (('c * ('a * 'c)) * ('a * 'b) list)
  type ('a, 'b, 'c) imst =
    IM_down of (('a, 'b, 'c) Find_state.find_state * ('b * ('a * 'b) list)) |
    IM_up of
      (('a, 'b, 'c) im_fo *
        ('a, 'c, unit) Searching_and_splitting.rsplit_node_ext list)
...
let rec mk_im_state
  k v kvs r = IM_down (Find_state.mk_find_state k r, (v, kvs));;

let rec dest_im_finished
  s = (match s with IM_down _ -> None | IM_up _ -> None
        | IM_finished (r, kvs) -> Some (r, kvs));;
...
let rec mk_insert_state k v r = I_down (Find_state.mk_find_state k r, v);;
...
let rec mk_delete_state k r = D_down (Find_state.mk_find_state k r, r);;
...
module Params : sig
  type 'a ps1 = Ps1 of (unit Prelude.constants_ext * ('a -> 'a -> Arith.int))
  val dummy : unit
  val dot_cmp : 'a ps1 -> 'a -> 'a -> Arith.int
  val dot_constants : 'a ps1 -> unit Prelude.constants_ext
end = struct
...
let rec dot_cmp
  ps1 = Util.rev_apply (Util.rev_apply ps1 dest_ps1) (fun (_, y) -> y);;

let rec dot_constants
  ps1 = Util.rev_apply (Util.rev_apply ps1 dest_ps1) (fun (x, _) -> x);;
...
let dummy : unit
  = Util.rev_apply
      (Find_state.dummy,
        (Delete_state.dummy,
          (Insert_many_state.dummy,
            (Insert_state.dummy, Leaf_stream_state.dummy))))
      (fun _ -> Params.dummy);;
...
module Store_ops : sig
  val store_free :
    ('a -> (('b, 'c, 'a) Disk_node.dnode, 'd) Monad.mm) *
      ((('b, 'c, 'a) Disk_node.dnode -> ('a, 'd) Monad.mm) *
        ('a list -> (unit, 'd) Monad.mm)) ->
      'a list -> (unit, 'd) Monad.mm
  val store_read :
    ('a -> (('b, 'c, 'a) Disk_node.dnode, 'd) Monad.mm) *
      ((('b, 'c, 'a) Disk_node.dnode -> ('a, 'd) Monad.mm) *
...
let rec store_free raf = let (_, (_, f)) = raf in
                         f;;

let rec store_read raf = let (r, (_, _)) = raf in
                         r;;
...
let rec wf_store_ops s = true;;

end;; (*struct Store_ops*)

module Find : sig
  val find_step :
    'a Params.ps1 ->
      ('b -> (('a, 'c, 'b) Disk_node.dnode, 'd) Monad.mm) *
        ((('a, 'c, 'b) Disk_node.dnode -> ('b, 'd) Monad.mm) *
          ('b list -> (unit, 'd) Monad.mm)) ->
  ...
  val wellformed_find_state :
    ('a -> 'a -> Arith.int) ->
      ('b -> 'c -> ('a, 'd) Tree.tree option) ->
        ('a, 'd) Tree.tree -> 'b -> ('a, 'd, 'c) Find_state.find_state -> bool
end = struct
  ...
  ps1 store_ops fs =
    (match fs
      with Find_state.F_down (r0, (k, (r, stk))) ->
        Util.rev_apply (Util.rev_apply store_ops Store_ops.store_read r)
          (Monad.fmap
            (fun a ->
              (match a
                with Disk_node.Disk_node (ks, rs) ->
                  let (stka, ra) =
                    ...
                    in
                  Find_state.F_down (r0, (k, (ra, stka)))
                | Disk_node.Disk_leaf kvs ->
                  Find_state.F_finished (r0, (k, (r, (kvs, stk)))))))
      | Find_state.F_finished _ -> Monad.return fs);;
    ...
    Util.assert_true
      (let _ = Tree.height t0 in
       ...
       let check_stack =
         ...
         in
       (match fs
         with Find_state.F_down (_, (k, (r, stk))) ->
           let (t_fo, t_stk) =
         ...
         | Find_state.F_finished (_, (k, (r, (_, stk)))) ->
           let (t_fo, t_stk) =
             Tree_stack.tree_to_rstack k_ord k t0 (List.size_list stk) in
           Util.assert_true (check_focus r t_fo) &&
             Util.assert_true (check_stack stk t_stk)));;
  ...
  val insert_step :
    'a Params.ps1 ->
      ('b -> (('a, 'c, 'b) Disk_node.dnode, 'd) Monad.mm) *
        ((('a, 'c, 'b) Disk_node.dnode -> ('b, 'd) Monad.mm) *
          ('b list -> (unit, 'd) Monad.mm)) ->
        ('a, 'c, 'b) Insert_state.insert_state ->
          (('a, 'c, 'b) Insert_state.insert_state, 'd) Monad.mm
  val wellformed_insert_state :
    unit Prelude.constants_ext ->
      ('a -> 'a -> Arith.int) ->
        ('b -> 'c -> ('a, 'd) Tree.tree option) ->
          ('a, 'd) Tree.tree ->
            'b -> 'a -> 'd -> ('a, 'd, 'c) Insert_state.insert_state -> bool
end = struct
    ...
    (match u with (_, []) -> Util.impossible1 "insert, step_up"
      | (Insert_state.I1 r, p :: stk) ->
        ...
        Util.rev_apply
          ...
          (Monad.fmap (fun ra -> (Insert_state.I1 ra, stk)))
      | (Insert_state.I2 (r1, (k, r2)), p :: stk) ->
        let (ks2, rs2) =
        ...
        (match
          ...
          with true ->
            Util.rev_apply
              (Util.rev_apply (Disk_node.mk_Disk_node (ks, rs))
                (Util.rev_apply store_ops Store_ops.store_alloc))
              (Monad.fmap (fun r -> (Insert_state.I1 r, stk)))
          | false ->
            let (ks_rs1, (ka, ks_rs2)) =
              Searching_and_splitting.split_node cs (ks, rs) in
            Util.rev_apply
              ...
              (Monad.bind
                (fun r1a ->
                  Util.rev_apply
                    ...
                    (Monad.fmap
                      (fun r2a ->
                        (Insert_state.I2 (r1a, (ka, r2a)), stk)))))));;
...
let rec step_bottom
  ps1 store_ops d =
    ...
    (match Find_state.dest_f_finished fs
      ...
      | Some (r0, (k, (_, (kvs, stk)))) ->
        Util.rev_apply
          (Util.rev_apply store_ops Store_ops.store_free
                  ...
                  with true ->
                    Util.rev_apply
                      (Util.rev_apply (Disk_node.Disk_leaf kvsa)
                        (Util.rev_apply store_ops Store_ops.store_alloc))
                      (Monad.fmap (fun a -> Insert_state.I1 a))
                  | false ->
                    let (kvs1, (ka, kvs2)) =
                        ...
                        (fun r1 ->
                          Util.rev_apply
                            (Util.rev_apply (Disk_node.Disk_leaf kvs2)
                              (Util.rev_apply store_ops Store_ops.store_alloc))
                            (Monad.fmap
                              (fun r2 -> Insert_state.I2 (r1, (ka, r2)))))))
                in
  ...
  ps1 store_ops s =
    (match s
      with Insert_state.I_down d ->
        ...
        (match Find_state.dest_f_finished fs
          with None ->
            Util.rev_apply (step_down ps1 store_ops d)
              (Monad.fmap (fun a -> Insert_state.I_down a))
          | Some _ ->
            Util.rev_apply (step_bottom ps1 store_ops d)
              (Monad.fmap (fun a -> Insert_state.I_up a)))
      | Insert_state.I_up u ->
        (match u
          ...
          | (Insert_state.I2 (r1, (k, r2)), []) ->
            Util.rev_apply
              (Util.rev_apply (Disk_node.mk_Disk_node ([k], [r1; r2]))
                (Util.rev_apply store_ops Store_ops.store_alloc))
              (Monad.fmap (fun a -> Insert_state.I_finished a))
          | (_, _ :: _) ->
            Util.rev_apply (step_up ps1 store_ops u)
              (Monad.fmap (fun a -> Insert_state.I_up a)))
      | Insert_state.I_finished _ -> Monad.return s);;

let rec wellformed_insert_state
  cs k_ord r2t t0 s k v is =
    Util.assert_true
      (match is with Insert_state.I_down a -> wf_d k_ord r2t t0 s a
        | Insert_state.I_up a -> wf_u r2t k_ord t0 s k v a
        | Insert_state.I_finished a -> wf_f cs k_ord r2t t0 s k v a);;
  ...
  val delete_step :
    'a Params.ps1 ->
      ('b -> (('a, 'c, 'b) Disk_node.dnode, 'd) Monad.mm) *
        ((('a, 'c, 'b) Disk_node.dnode -> ('b, 'd) Monad.mm) *
          ('b list -> (unit, 'd) Monad.mm)) ->
        ('a, 'c, 'b) Delete_state.delete_state ->
          (('a, 'c, 'b) Delete_state.delete_state, 'd) Monad.mm
  val wellformed_delete_state :
    unit Prelude.constants_ext ->
      ('a -> 'a -> Arith.int) ->
        ('b -> 'c -> ('a, 'd) Tree.tree option) ->
          ('a, 'd) Tree.tree ->
            'b -> 'a -> ('a, 'd, 'c) Delete_state.delete_state -> bool
end = struct
...
let rec step_up
  ps1 store_ops du =
    ...
    let (alloc, read) =
      (Util.rev_apply store_ops Store_ops.store_alloc,
        Util.rev_apply store_ops Store_ops.store_read)
      in
...
let rec delete_step
  ps1 store_ops s =
    let alloc = Util.rev_apply store_ops Store_ops.store_alloc in
    (match s
      with Delete_state.D_down (f, r0) ->
        (match Find_state.dest_f_finished f
          with None ->
            Util.rev_apply (Find.find_step ps1 store_ops f)
      ...
      | Delete_state.D_up (f, (stk, r0)) ->
        (match Util.is_Nil stk
            ...
            (match f
              with Delete_state.D_small_leaf kvs ->
                Util.rev_apply (Util.rev_apply (Disk_node.Disk_leaf kvs) alloc)
                  (Monad.fmap (fun a -> Delete_state.D_finished a))
              | Delete_state.D_small_node (ks, rs) ->
                (match Arith.equal_nata (List.size_list ks) Arith.zero_nat
                  with true ->
                    Monad.return (Delete_state.D_finished (List.hd rs))
                  | false ->
                    Util.rev_apply
          ...
          | false ->
            Util.rev_apply (step_up ps1 store_ops (f, stk))
              (Monad.fmap
                (fun (fa, stka) -> Delete_state.D_up (fa, (stka, r0)))))
      | Delete_state.D_finished _ -> Monad.return s);;

let rec wellformed_delete_state
  constants k_ord r2t t0 s k ds =
    Util.assert_true
      (match ds with Delete_state.D_down a -> wf_d k_ord r2t t0 s a
        | Delete_state.D_up (fo, (stk, r)) ->
          wf_u constants k_ord r2t t0 s k (fo, stk) &&
            (match r2t s r with None -> false | Some t -> Tree.tree_equal t t0)
        | Delete_state.D_finished a -> wf_f constants k_ord r2t t0 s k a);;
  ...
  val insert_step :
    'a Params.ps1 ->
      ('b -> (('a, 'c, 'b) Disk_node.dnode, 'd) Monad.mm) *
        ((('a, 'c, 'b) Disk_node.dnode -> ('b, 'd) Monad.mm) *
          ('b list -> (unit, 'd) Monad.mm)) ->
        ('a, 'c, 'b) Insert_many_state.imst ->
          (('a, 'c, 'b) Insert_many_state.imst, 'd) Monad.mm
end = struct
...
let rec step_up
  ps1 store_ops u =
    let (cs, _) =
      ...
      in
    (match u with (_, []) -> Util.impossible1 "insert, step_up"
      | (Insert_many_state.IM1 (r, kvs0), x :: stk) ->
        let (ks, rs) =
          Searching_and_splitting.unsplit_node
        ...
        Util.rev_apply
          (Util.rev_apply (Disk_node.mk_Disk_node (ks, rs))
            (Util.rev_apply store_ops Store_ops.store_alloc))
          (Monad.fmap (fun ra -> (Insert_many_state.IM1 (ra, kvs0), stk)))
      | (Insert_many_state.IM2 ((r1, (k, r2)), kvs0), x :: stk) ->
        let (ks2, rs2) =
        ...
        (match
          ...
          with true ->
            Util.rev_apply
              (Util.rev_apply (Disk_node.mk_Disk_node (ks, rs))
                (Util.rev_apply store_ops Store_ops.store_alloc))
              (Monad.fmap (fun r -> (Insert_many_state.IM1 (r, kvs0), stk)))
          | false ->
            let (ks_rs1, (ka, ks_rs2)) =
              Searching_and_splitting.split_node cs (ks, rs) in
            Util.rev_apply
              ...
              (Monad.bind
                (fun r1a ->
                  Util.rev_apply
                    ...
                    (Monad.fmap
                      (fun r2a ->
                        (Insert_many_state.IM2 ((r1a, (ka, r2a)), kvs0),
                          stk)))))));;
...
let rec insert_step
  ps1 store_ops s =
    let (_, _) =
      ...
      in
    (match s
      with Insert_many_state.IM_down d ->
        let (fs, (_, _)) = d in
        (match Find_state.dest_f_finished fs
          with None ->
            Util.rev_apply (step_down ps1 store_ops d)
              (Monad.fmap (fun a -> Insert_many_state.IM_down a))
          | Some _ ->
            Util.rev_apply (step_bottom ps1 store_ops d)
              (Monad.fmap (fun a -> Insert_many_state.IM_up a)))
      | Insert_many_state.IM_up u ->
        (match u
          with (Insert_many_state.IM1 (r, kvs0), []) ->
            Monad.return (Insert_many_state.IM_finished (r, kvs0))
          | (Insert_many_state.IM2 ((r1, (k, r2)), kvs0), []) ->
            Util.rev_apply
              (Util.rev_apply (Disk_node.mk_Disk_node ([k], [r1; r2]))
                (Util.rev_apply store_ops Store_ops.store_alloc))
              (Monad.fmap (fun r -> Insert_many_state.IM_finished (r, kvs0)))
          | (_, _ :: _) ->
            Util.rev_apply (step_up ps1 store_ops u)
              (Monad.fmap (fun a -> Insert_many_state.IM_up a)))
      | Insert_many_state.IM_finished _ -> Monad.return s);;
  ...
  val lss_step :
    'a Params.ps1 ->
      ('b -> (('a, 'c, 'b) Disk_node.dnode, 'd) Monad.mm) *
        ((('a, 'c, 'b) Disk_node.dnode -> ('b, 'd) Monad.mm) *
          ('b list -> (unit, 'd) Monad.mm)) ->
        ('a, 'c, 'b) Leaf_stream_state.ls_state ->
          (('a, 'c, 'b) Leaf_stream_state.ls_state, 'd) Monad.mm
  val dest_LS_leaf :
    ('a, 'b, 'c) Leaf_stream_state.ls_state -> (('a * 'b) list) option
  val lss_is_finished : ('a, 'b, 'c) Leaf_stream_state.ls_state -> bool
end = struct
      ...
      (Monad.fmap
        (fun a ->
          (match a
            ...
            | Disk_node.Disk_leaf kvs ->
              Leaf_stream_state.LS_leaf (kvs, fs))));;

let rec lss_step
  ps1 store_ops lss =
    (match lss with Leaf_stream_state.LS_down a -> step_down ps1 store_ops a
      | Leaf_stream_state.LS_leaf x -> Monad.return (step_leaf x)
      | Leaf_stream_state.LS_up x -> Monad.return (step_up x));;

let rec dest_LS_leaf
  x = (match x with Leaf_stream_state.LS_down _ -> None
        | Leaf_stream_state.LS_leaf (kvs, _) -> Some kvs
        | Leaf_stream_state.LS_up _ -> None);;

let rec lss_is_finished
  lss = (match lss with Leaf_stream_state.LS_down _ -> false
          | Leaf_stream_state.LS_leaf _ -> false
          | Leaf_stream_state.LS_up [] -> true
          | Leaf_stream_state.LS_up (_ :: _) -> false);;
