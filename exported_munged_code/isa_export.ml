...
module Find : sig
  ...
  val find_step :
    'a Params.ps1 ->
      ('a, 'b, 'c, 'd, unit) Store_ops.store_ops_ext ->
        ...
        (match Find.dest_f_finished fs
          with None ->
            Util.rev_apply (step_down ps1 store_ops d)
              (Monad.fmap (fun a -> I_down a))
          | Some _ ->
            Util.rev_apply (step_bottom ps1 store_ops d)
              (Monad.fmap (fun a -> I_up a)))
      | I_up u ->
...
let rec lss_step
  ps1 store_ops lss =
    (match lss with LS_down a -> step_down ps1 store_ops a
