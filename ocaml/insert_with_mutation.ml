...
module Pre_monad : sig
  ...
  val dest_Frm : ('a, 'b, 'c) stk_frame -> 'a * ('b * 'c)
  ...
  val make_frame :
    ('a -> 'a -> Arith.int) ->
      'a -> 'a list ->
              'b list ->
                (('b list * 'a list), 'b, ('a list * 'b list)) stk_frame * 'b
                ...
                (match fo
                  with Pre_monad.I1 r ->
                    (let (ks, rs) =
                       ...
                       in
                      Pre_monad.rev_apply
                        (Pre_monad.rev_apply (Pre_monad.Disk_node (ks, rs))
                          dnode2blka)
                        (fun blk ->
                          Pre_monad.rev_apply (write blk)
                            (Monad.bind
                              (fun ra -> Monad.return (Pre_monad.I1 ra, stk)))))
                          ...
                          Pre_monad.rev_apply
                            ...
                            (fun blk ->
                              Pre_monad.rev_apply (write blk)
                                (Monad.bind
                                  (fun r ->
                                    Monad.return (Pre_monad.I1 r, stk))))
                        | false ->
                          (let (ks_rs1, (ka, ks_rs2)) =
                             Pre_monad.split_node cs (ks, rs) in
                            Pre_monad.rev_apply
                              (Pre_monad.rev_apply (Pre_monad.Disk_node ks_rs1)
                                dnode2blka)
                              (fun blk ->
                                Pre_monad.rev_apply (write blk)
                                  (Monad.bind
...
(fun blka ->
  Pre_monad.rev_apply (write blka)
    (Monad.bind
      (fun r2a -> Monad.return (Pre_monad.I2 (r1a, (ka, r2a)), stk)))))))))))))
              b)));;
                ...
                (fun a ->
                  (match a
                    with Pre_monad.Disk_node (ks, rs) ->
                      (let (frm, ra) = Pre_monad.make_frame k_cmp k ks rs in
            ...
            (match u
              ...
              | (_, _ :: _) ->
                Pre_monad.rev_apply (step_upa u)
                  (Monad.fmap (fun a -> Pre_monad.I_up a)))
