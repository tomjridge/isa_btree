                              ...
                              (let (ks2, _) = ad in
                                (fun _ ->
                                  (let la =
                                     (match l
                                       with None ->
 (match ks1 with [] -> None | k :: _ -> Some k)
                                       | Some _ -> l)
                                     in
                                   let ua =
                                     (match u
                                       with None ->
 (match ks2 with [] -> None | k :: _ -> Some k)
                                       | Some _ -> u)
                                     in
                                    Some (la, (ua, stka)))))
                                ba)))
...
module Leaf_stream_state : sig
  type ('a, 'b, 'c) lss =
    LS_down of
      ('c * (('c list * 'a list), 'c, ('a list * 'c list), 'c)
...
let rec make_initial_lss r = LS_down (r, []);;
...
module Insert_many_state : sig
  ...
  val make_initial_im_state :
    'a -> 'b -> 'c -> ('b * 'c) list ->
                        ('b, 'c, 'a) Insert_state.insert_state * ('b * 'c) list
end = struct
...
module Post_monad : sig
  ...
  val check_tree_at_r :
    Constants_and_size_types.constants ->
      ('a -> 'a -> Arith.int) ->
        ('b, ('a, 'c, 'b) Disk_node.dnode, 'd) store_ops ->
          'b -> (unit, 'd) Monad.mm
                ...
                (iter_m
                  (fun aa ->
                    (match aa with (_, []) -> Monad.return None
                      | (ts, ra :: rsa) ->
                        A_start_here.rev_apply (get_tree store_ops ra)
                          (Monad.bind
                            (fun t -> Monad.return (Some (t :: ts, rsa))))))
                  ([], rs))
                (Monad.bind
                  (fun (ts, _) -> Monad.return (Tree.Node (ks, Lista.rev ts))))
            | Disk_node.Disk_leaf kvs -> Monad.return (Tree.Leaf kvs))));;
...
let rec check_tree_at_r
  cs k_cmp store_ops r =
    (match A_start_here.get_check_flag ()
      with true ->
        A_start_here.rev_apply (get_tree store_ops r)
          (Monad.bind
            (fun t ->
              (let _ =
                 A_start_here.check_true (fun _ -> Tree.wf_tree cs None k_cmp t)
                 in
                Monad.return ())))
      | false -> Monad.return ());;
...
let rec leaf_steal_right
  store_ops =
    (let write = A_start_here.rev_apply store_ops Post_monad.wrte in
      (fun c1 (kv1 :: kv2 :: rest) ->
        A_start_here.rev_apply
          (A_start_here.rev_apply (Disk_node.Disk_leaf (c1 @ [kv1])) write)
          (Monad.bind
            (fun left ->
              A_start_here.rev_apply
                (A_start_here.rev_apply (Disk_node.Disk_leaf (kv2 :: rest))
                  write)
                (Monad.bind
                  (fun right ->
...
let rec leaf_merge_right
  store_ops =
    (let write = A_start_here.rev_apply store_ops Post_monad.wrte in
      (fun c1 c2 ->
        (let kvs = c1 @ c2 in
          A_start_here.rev_apply (Disk_node.Disk_leaf kvs) write)));;
    ...
    (let write = A_start_here.rev_apply store_ops Post_monad.wrte in
      (fun c1 c2 ->
        (let kv1 :: rest = Lista.rev c1 in
          A_start_here.rev_apply
            (A_start_here.rev_apply (Disk_node.Disk_leaf (Lista.rev rest))
              write)
            (Monad.bind
              (fun left ->
                A_start_here.rev_apply
                  (A_start_here.rev_apply (Disk_node.Disk_leaf (kv1 :: c2))
                    write)
                  (Monad.bind
                    (fun right ->
                      Monad.return
...
let rec leaf_merge_left store_ops = leaf_merge_right store_ops;;
                       ...
                       let (r1 :: rs1, _ :: ks1) = rks in
                        A_start_here.rev_apply
                          ...
                          (Monad.bind
                            (fun left_kvs ->
                              (match
                                Arith.equal_nata (Lista.size_list left_kvs)
                                  (A_start_here.rev_apply cs
                                    Constants_and_size_types.min_leaf_size)
                                with true ->
                                  A_start_here.rev_apply
                                    (leaf_merge_left store_ops left_kvs kvs)
                                    (Monad.bind
                                      (fun r ->
A_start_here.rev_apply
  (Stacks_and_frames.unsplit_node ((rs1, ks1), (([r], []), krs))) post_mergea))
                                | false ->
                                  A_start_here.rev_apply
                                    (leaf_steal_left store_ops left_kvs kvs)
                                    (Monad.bind
                                      (fun (r1a, (k1, r2)) ->
A_start_here.rev_apply
  (A_start_here.rev_apply
                       ...
                       let (_ :: ks1, r1 :: rs1) = krs in
                        A_start_here.rev_apply
                          ...
                          (Monad.bind
                            (fun right_kvs ->
                              (match
                                Arith.equal_nata (Lista.size_list right_kvs)
                                  (A_start_here.rev_apply cs
                                    Constants_and_size_types.min_leaf_size)
                                with true ->
                                  A_start_here.rev_apply
                                    (leaf_merge_right store_ops kvs right_kvs)
                                    (Monad.bind
                                      (fun r ->
A_start_here.rev_apply
  (Stacks_and_frames.unsplit_node (rks, (([r], []), (ks1, rs1)))) post_mergea))
                                | false ->
                                  A_start_here.rev_apply
                                    (leaf_steal_right store_ops kvs right_kvs)
                                    (Monad.bind
                                      (fun (r1a, (k1, r2)) ->
A_start_here.rev_apply
  (A_start_here.rev_apply
        ...
        (match a
          with Delete_state.D_down (f, r0) ->
            (match Find_state.dest_F_finished f
              with None ->
                A_start_here.rev_apply (Find.find_step cs k_cmp store_ops f)
                  ...
                  with true ->
                    (let kvsa =
                      ...
                      (match
                        ...
                        | false ->
                          A_start_here.rev_apply
                            (A_start_here.rev_apply (Disk_node.Disk_leaf kvsa)
                              write)
                            (Monad.fmap
                              (fun r ->
                                Delete_state.D_up
          ...
          | Delete_state.D_up (f, (stk, r0)) ->
            (match A_start_here.is_Nil stk
              ...
              | false ->
                A_start_here.rev_apply (step_up cs store_ops (f, stk))
                  (Monad.fmap
...
let rec delete_big_step
  cs k_cmp store_ops =
    (let delete_stepa = delete_step cs k_cmp store_ops in
      Post_monad.iter_m
        (fun d ->
          (match d
...
let rec delete
  cs k_cmp store_ops r k =
    (let check_tree_at_r = Post_monad.check_tree_at_r cs k_cmp store_ops in
     let d = Delete_state.make_initial_delete_state r k in
      A_start_here.rev_apply (delete_big_step cs k_cmp store_ops d)
        (Monad.bind
          (fun a ->
            (match a
              with Delete_state.D_down _ ->
...
module Insert : sig
  ...
  val insert :
    Constants_and_size_types.constants ->
      ('a -> 'a -> Arith.int) ->
        ('b, ('a, 'c, 'b) Disk_node.dnode, 'd) Post_monad.store_ops ->
          'b -> 'a -> 'c -> (('b option), 'd) Monad.mm
end = struct

let rec step_bottom
  cs k_cmp store_ops d =
    (let (write, rewrite) =
       (A_start_here.rev_apply store_ops Post_monad.wrte,
     ...
     let (fs, v) = d in
      (match Find_state.dest_F_finished fs
        ...
        | Some (_, (k, (r, (kvs, stk)))) ->
          (let kvsa =
             A_start_here.rev_apply kvs (Key_value.kvs_insert k_cmp k v) in
            (match
              Arith.less_eq_nat (Lista.size_list kvsa)
                (A_start_here.rev_apply cs
                  Constants_and_size_types.max_leaf_size)
              ...
              | false ->
                (let (kvs1, (ka, kvs2)) = Insert_state.split_leaf cs kvsa in
                  A_start_here.rev_apply
                    (A_start_here.rev_apply (Disk_node.Disk_leaf kvs1) write)
                    (Monad.bind
                      (fun r1 ->
                        A_start_here.rev_apply
                          (A_start_here.rev_apply (Disk_node.Disk_leaf kvs2)
                            write)
                          (Monad.bind
                            (fun r2 ->
                              Monad.return
...
let rec step_down
  cs k_cmp store_ops =
    (let find_step = Find.find_step cs k_cmp store_ops in
      (fun (fs, v) ->
...
let rec insert_step
  cs k_cmp store_ops =
    (let step_downa = step_down cs k_cmp store_ops in
     let step_bottoma = step_bottom cs k_cmp store_ops in
...
let rec insert_big_step
  cs k_cmp store_ops =
    (let insert_stepa = insert_step cs k_cmp store_ops in
      Post_monad.iter_m
        (fun i ->
          (match i
...
let rec insert
  cs k_cmp store_ops r k v =
    (let check_tree_at_r = Post_monad.check_tree_at_r cs k_cmp store_ops in
     let i = Insert_state.make_initial_insert_state r k v in
      A_start_here.rev_apply (insert_big_step cs k_cmp store_ops i)
        (Monad.bind
          (fun a ->
            (match a
...
module Insert_many : sig
  val im_step :
    Constants_and_size_types.constants ->
      ('a -> 'a -> Arith.int) ->
        ('b, ('a, 'c, 'b) Disk_node.dnode, 'd) Post_monad.store_ops ->
          ('a, 'c, 'b) Insert_state.insert_state * ('a * 'c) list ->
            ((('a, 'c, 'b) Insert_state.insert_state * ('a * 'c) list), 'd)
              Monad.mm
end = struct
                         ...
                         | Key_value.EQ ->
                           Some ((k, v) :: result,
                                  (Arith.plus_nat len_result Arith.one_nat,
                                    (newaa,
                                      (existingaa,
Arith.minus_nat len_existing Arith.one_nat))))
                         | Key_value.GT ->
                           Some ((ka, va) :: result,
                                  (Arith.plus_nat len_result Arith.one_nat,
                                    (newb,
...
Arith.minus_nat len_existing Arith.one_nat))))))))))
     ...
     let (result, (_, (newb, (existinga, _)))) =
       A_start_here.iter_step step
         ([], (Arith.zero_nat,
                (kv :: newa, (existing, Lista.size_list existing))))
       in
     let resulta = Lista.rev result @ existinga in
      (resulta, newb));;

let rec im_step_bottom
  cs k_cmp store_ops d kvs0 =
    (let (fs, v) = d in
      (match Find_state.dest_F_finished fs
        with None -> A_start_here.impossible1 "insert, step_bottom"
        | Some (_, (k, (_, (kvs, stk)))) ->
          (let (_, u) = Stacks_and_frames.get_bounds stk in
           let (kvsa, kvs0a) = kvs_insert_2 cs k_cmp u (k, v) kvs0 kvs in
            (match
              Arith.less_eq_nat (Lista.size_list kvsa)
                (A_start_here.rev_apply cs
                  Constants_and_size_types.max_leaf_size)
              with true ->
                A_start_here.rev_apply
                  (A_start_here.rev_apply (Disk_node.Disk_leaf kvsa)
                    (A_start_here.rev_apply store_ops Post_monad.wrte))
                  (Monad.fmap (fun r -> ((Insert_state.I1 r, stk), kvs0a)))
              | false ->
                (let (kvs1, (ka, kvs2)) = Insert_state.split_leaf cs kvsa in
                  A_start_here.rev_apply
                    (A_start_here.rev_apply (Disk_node.Disk_leaf kvs1)
                      (A_start_here.rev_apply store_ops Post_monad.wrte))
                    (Monad.bind
                      (fun r1 ->
                        A_start_here.rev_apply
      ...
      (match i
        with Insert_state.I_down d ->
          (let (fs, _) = d in
            (match Find_state.dest_F_finished fs
              with None ->
                A_start_here.rev_apply (Insert.insert_step cs k_cmp store_ops i)
                  (Monad.fmap (fun da -> (da, kvs)))
              | Some _ ->
                A_start_here.rev_apply (im_step_bottom cs k_cmp store_ops d kvs)
...
module Leaf_stream : sig
  val lss_step :
    ('a -> 'a -> Arith.int) ->
      ('b, ('a, 'c, 'b) Disk_node.dnode, 'd) Post_monad.store_ops ->
        ('a, 'c, 'b) Leaf_stream_state.lss ->
          (('a, 'c, 'b) Leaf_stream_state.lss, 'd) Monad.mm
end = struct

let rec step_up
  fs = (let _ = A_start_here.check_true (fun _ -> not (A_start_here.is_Nil fs))
          in
         (match fs with [] -> A_start_here.failwitha "Leaf_stream, step_up"
           | f :: fsa ->
             (match A_start_here.rev_apply f Stacks_and_frames.dest_Frm
               with (_, (_, (([], []), _))) -> Leaf_stream_state.LS_up fsa
               | (_, (_, ((_ :: ks1, r1 :: rs1), r))) ->
                 (let fa =
                    Stacks_and_frames.Frm (([], []), (r1, ((ks1, rs1), r))) in
                   Leaf_stream_state.LS_down (r1, fa :: fsa)))));;
          ...
          (fun a ->
            (match a
              with Disk_node.Disk_node (ks, rs) ->
                (let _ =
                   A_start_here.check_true
                     (fun _ -> not (A_start_here.is_Nil rs))
                   in
                 let ra :: rsa = rs in
                 let frm =
                   Stacks_and_frames.Frm (([], []), (ra, ((ks, rsa), r))) in
...
let rec lss_step
  k_cmp store_ops lss =
    (match lss with Leaf_stream_state.LS_down a -> step_down k_cmp store_ops a
