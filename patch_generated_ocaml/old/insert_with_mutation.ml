...
module Insert_with_mutation(Monad:MONAD) : sig
  ...
  val insert_step :
    Pre_monad.constants ->
      ('a -> 'a -> Arith.int) ->
        ('b, ('a, 'c, 'b) Pre_monad.dnode, 'd) store_ops ->
          ('a, 'c, 'b) Pre_monad.insert_state ->
            (('a, 'c, 'b) Pre_monad.insert_state, 'd) Monad.mm
end = struct
...
let rec rewrite (Make_store_ops (x1, x2, x3)) = x3;;

let rec wrte (Make_store_ops (x1, x2, x3)) = x2;;

let rec step_up
  cs k_cmp store_ops u =
    (let (write, rewritea) =
       (Pre_monad.rev_apply store_ops wrte,
         Pre_monad.rev_apply store_ops rewrite)
       in
                              ...
                              (Monad.bind
                                (fun ad ->
                                  (match ad
                                    ...
                                    | Some r2 ->
                                      Monad.return
(Sum_Type.Inl (Pre_monad.I1 r2, stk))))))
...
| Some r2a -> Monad.return (Sum_Type.Inl (Pre_monad.I1 r2a, stk)))))
                              | false ->
                                (let (ks_rs1, (ka, ks_rs2)) =
                                  ...
                                  Pre_monad.rev_apply
                                    (Pre_monad.rev_apply
                                      (Pre_monad.Disk_node ks_rs1) write)
                                    (Monad.bind
                                      (fun r1a ->
Pre_monad.rev_apply (Pre_monad.rev_apply (Pre_monad.Disk_node ks_rs2) write)
  (Monad.bind
    (fun r2a ->
      Monad.return (Sum_Type.Inl (Pre_monad.I2 (r1a, (ka, r2a)), stk))))))))))))
                    ba)))
              b)));;

let rec read (Make_store_ops (x1, x2, x3)) = x1;;

let rec find_step
  cs k_cmp store_ops =
    (let reada = Pre_monad.rev_apply store_ops read in
      (fun fs ->
        (match fs
          with Pre_monad.F_down (r0, (k, (r, stk))) ->
            Pre_monad.rev_apply (reada r)
              (Monad.fmap
                (fun a ->
                  (match a
...
let rec step_down
  cs k_cmp store_ops =
    (let find_stepa = find_step cs k_cmp store_ops in
      (fun (fs, v) ->
        ...
        | Some (_, (k, (r, (kvs, stk)))) ->
          (let kvsa = Pre_monad.rev_apply kvs (Pre_monad.kvs_insert k_cmp k v)
            ...
            (match
              ...
              with true ->
                Pre_monad.rev_apply
                  (Pre_monad.rev_apply (Pre_monad.Disk_leaf kvsa) (rewritea r))
                  (Monad.bind
                    (fun a ->
                      (match a with None -> Monad.return (Sum_Type.Inr ())
                        | Some ra ->
                          Monad.return (Sum_Type.Inl (Pre_monad.I1 ra, stk)))))
              | false ->
                (let (kvs1, (ka, kvs2)) = Pre_monad.split_leaf cs kvsa in
                  Pre_monad.rev_apply
                    (Pre_monad.rev_apply (Pre_monad.Disk_leaf kvs1) write)
                    (Monad.bind
                      (fun r1 ->
                        Pre_monad.rev_apply
                          (Pre_monad.rev_apply (Pre_monad.Disk_leaf kvs2) write)
                          (Monad.bind
                            (fun r2 ->
                              Monad.return
                                (Sum_Type.Inl
                                  (Pre_monad.I2 (r1, (ka, r2)), stk)))))))))));;
...
let rec insert_step
  cs k_cmp store_ops =
    (let step_downa = step_down cs k_cmp store_ops in
     ...
     let write = Pre_monad.rev_apply store_ops wrte in
      (fun s ->
        (match s
              ...
              | (Pre_monad.I2 (r1, (k, r2)), []) ->
                Pre_monad.rev_apply
                  (Pre_monad.rev_apply (Pre_monad.Disk_node ([k], [r1; r2]))
                    write)
                  (Monad.bind (fun r -> Monad.return (Pre_monad.I_finished r)))
              | (_, _ :: _) ->
