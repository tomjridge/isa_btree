...
Leaf xs \<Rightarrow> (
let n = length xs in
(n \<ge> min_leaf_size) & ( n \<le> max_leaf_size)) (*I leave the max_leaf_size because it does not concert the inserting_two \<rightarrow> inserting_two case*)
...
let b1 = wf_size' rmbs t0 in (*we use a wf_size def without upper limit, so our node can be fat*)
...
lemma wf_size_1_implies_wf_size_1': " wf_size_1 t0 \<Longrightarrow> wf_size_1' t0"
apply (simp add:wf_size_1_def wf_size_1'_def)
apply (case_tac t0)
 apply (simp,case_tac x1, force simp add:Let_def)

 apply force
done

lemma wf_size_implies_wf_size': " wf_size rmbs t \<Longrightarrow> wf_size' rmbs t"
apply (simp add:wf_size'_def wf_size_def)
apply (case_tac rmbs)
 apply (simp,case_tac x,simp,case_tac t, simp,case_tac x1,simp)
 apply (force simp add: wf_size_1_implies_wf_size_1' list_all_length)+
 
 apply (simp,case_tac x,simp,case_tac t, simp,case_tac x1,simp)
 apply (force simp add: forall_subtrees_def wf_size_1_implies_wf_size_1' list_all_length)+
done
...
apply(intro conjI)
 ...
 apply(case_tac f)
  ...
  apply (erule exE)
  apply (subgoal_tac "wellformed_tree' (Rmbs (stk = [])) fat_node") 
  prefer 2
   ...
   apply (intro conjI)
    ...
    apply (case_tac "stk=[]")
     (*stk = []*)
     apply (subgoal_tac "list_all wf_size_1' rs")
     prefer 2
      apply (force simp add:list_all_length wf_size_1_implies_wf_size_1')
     ...
     prefer 2
      ...
      apply (force simp add: wf_size_1_implies_wf_size_1')
     apply (force simp add:list_all_of_list_replace_at_n)
    
     (*stk \<noteq> []*)
     apply (simp add:forall_subtrees_def rev_apply_def)
     apply (simp add:wf_size_1'_def wf_size_1_def Let_def)
     apply (subgoal_tac "
       list_all wf_size_1' (concat (map tree_to_subtrees rs)) \<and>
       list_all wf_size_1' (tree_to_subtrees tleft) \<and> 
       list_all wf_size_1' (tree_to_subtrees tright)")
     prefer 2
      apply (force simp add:list_all_length wf_size_1_implies_wf_size_1')
     apply (force simp add:list_all_of_list_replace_at_n_concat_map_subtrees)
  ...
  (*FIXME fix remaining of the proof*)
